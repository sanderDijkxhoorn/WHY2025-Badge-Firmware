...
// Connect to a specific SSID with optional password (empty string for open networks).
// Returns WIFI_CONNECTED on success, WIFI_ERROR on failure or timeout.
wifi_connection_status_t wifi_connect_to(char const *ssid, char const *password);

// Get the authentication mode of a scanned station (e.g., WIFI_AUTH_OPEN vs WPA2/WPA3).
wifi_auth_mode_t wifi_station_get_authmode(wifi_station_handle station);
